...
public class AllureEnvironmentWriterUtil {
	...
	public static void createEnvironmentPropertiesFile ()  {
		...
		prop.setProperty("Project name ", "Phoneix Automation Framework");
		...
		try {
			...
			prop.store(fw, "Praveen properties file");
			LOGGER.info("Created the environment.properties file at {}",folderPath);
		} catch (IOException e) {
			LOGGER.error("Unavle to create envirnment.properties file",e);
			e.printStackTrace();
		}
